...
class UnitTestOpenAIParser(UnitTestCase):
	def test_extract_fields_sends_json_schema_and_parses_response(self):
		...
		result = OpenAIParser(client=fake_client).extract_fields("some document text", FIELD_SPECS)
		...
		self.assertEqual(call_kwargs["model"], "gpt-4.1")
	...
	def test_extract_rows_sends_array_schema_and_parses_response(self):
		...
		result = OpenAIParser(client=fake_client).extract_rows("some table text", FIELD_SPECS)
